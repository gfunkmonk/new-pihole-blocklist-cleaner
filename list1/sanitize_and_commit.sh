...
input_file="$workspace/list1/blocklists.txt"
date_str=$(date -u +'%Y-%m-%d')
output_versioned="$workspace/list1/blocklist_${date_str}.txt"
output_static="$workspace/list1/blocklist.txt"
...
if [[ -n "$GITHUB_ACTIONS" ]]; then
  ...
  for f in "$workspace"/list1/blocklist_[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].txt; do
    [[ -f "$f" ]] || continue
    fname=$(basename "$f")
